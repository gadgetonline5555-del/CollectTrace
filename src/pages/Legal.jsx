...
export default function Legal() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
      <Link to="/settings" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground mb-6">
      ...
      <section id="privacy" className="rounded-2xl border border-border bg-card/60 p-6 mb-6">
        <h1 className="font-display text-2xl font-bold text-foreground mb-4">{jp ? "プライバシーポリシー" : "Privacy Policy"}</h1>
      ...
      <section id="terms" className="rounded-2xl border border-border bg-card/60 p-6">
        <h1 className="font-display text-2xl font-bold text-foreground mb-4">{jp ? "利用規約" : "Terms of Service"}</h1>
        <div className="space-y-3">
          {terms.map((p, i) => (
            <p key={i} className="text-sm text-muted-foreground leading-relaxed">{p}</p>
          ))}
