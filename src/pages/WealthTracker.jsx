        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {PTYPES.map((pt) => (
            <button key={pt.id} type="button" onClick={() => setProfileType(pt.id)}
              className={`min-h-11 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${profileType === pt.id ? "bg-white text-slate-950" : "bg-slate-900 text-slate-300 hover:bg-slate-800 border border-white/10"}`}>
              {t(pt.key)}
            </button>
          ))}
        </div>
          ...
          <div className="flex items-center justify-between gap-3 mb-5 flex-wrap">
            <div className="flex items-center gap-2">
              <button onClick={handleRefresh} disabled={refreshing}
                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold bg-gradient-to-r from-amber-400 to-orange-500 text-slate-950 hover:opacity-90 disabled:opacity-50 transition-opacity">
          ...
          {refreshing ? (
            ...
            <div className="rounded-3xl border border-white/10 bg-slate-900/60 p-12 text-center">
              ...
              <button onClick={handleRefresh}
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold bg-gradient-to-r from-amber-400 to-orange-500 text-slate-950 hover:opacity-90">
              ...
              <div className="space-y-2">
                {history.map((h) => (
                  <button key={h.id} onClick={() => setViewing(h)}
                    className={`w-full text-left flex items-center justify-between gap-3 px-4 py-3 rounded-xl border transition-colors ${viewing?.id === h.id ? "border-amber-400/40 bg-amber-500/10" : "border-white/10 bg-slate-900/50 hover:bg-slate-800/50"}`}>
