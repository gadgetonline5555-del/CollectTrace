  ...
  return (
    <TierGate requiredTier="starter" title={t("wl.locked_h")} description={t("wl.locked_p")}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
        ...
        <form onSubmit={add} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 mb-8 p-5 rounded-2xl bg-slate-900/60 border border-white/10">
          ...
          <div className="space-y-3">
            {items.map((it) => (
              <div key={it.id} className="flex items-center gap-4 p-4 rounded-xl bg-slate-900/60 border border-white/10">
                ...
                <button onClick={() => remove(it.id)} aria-label={t("wl.remove")} className="min-h-11 min-w-11 inline-flex items-center justify-center text-slate-500 hover:text-rose-400"><Trash2 className="w-4 h-4" /></button>
              </div>
            ))}
          </div>
        )}
