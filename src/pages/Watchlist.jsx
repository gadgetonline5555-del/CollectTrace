    ...
    <TierGate requiredTier="starter" title={t("wl.locked_h")} description={t("wl.locked_p")}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-2">{t("wl.p")}</p>
        </div>
        <form onSubmit={add} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 mb-8 p-5 rounded-2xl bg-card/60 border border-border">
          <input value={form.ticker} onChange={(e) => setForm({ ...form, ticker: e.target.value })} placeholder={t("wl.ticker")} className="px-3 py-2 rounded-lg bg-background border border-border text-foreground text-sm" />
          <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder={t("wl.title")} className="px-3 py-2 rounded-lg bg-background border border-border text-foreground text-sm" />
          <input value={form.target_price} onChange={(e) => setForm({ ...form, target_price: e.target.value })} placeholder={t("wl.target")} type="number" className="px-3 py-2 rounded-lg bg-background border border-border text-foreground text-sm" />
          <div className="flex gap-2">
            <input value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} placeholder={t("wl.note")} className="flex-1 px-3 py-2 rounded-lg bg-background border border-border text-foreground text-sm" />
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {items.map((it) => (
              <div key={it.id} className="flex items-center gap-4 p-4 rounded-xl bg-card/60 border border-border">
