...
export default function Invite() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
      ...
      <h1 className="font-display text-4xl font-bold text-foreground">{t("invite.h")}</h1>
      <p className="text-muted-foreground mt-2 max-w-2xl">{t("invite.p")}</p>
      ...
      {!user?.id ? (
        <div className="mt-6 rounded-2xl border border-border bg-card/60 p-8 text-center">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <Link to="/login" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold bg-slate-800 text-foreground border border-border hover:bg-slate-700">
              {t("invite.login")}
            </Link>
          </div>
        </div>
