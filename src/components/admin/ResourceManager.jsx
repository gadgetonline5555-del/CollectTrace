  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <p className="text-muted-foreground text-sm">{items.length}件</p>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto bg-background border-border">
          <DialogHeader><DialogTitle className="text-foreground">{editing?.id ? "編集" : "新規追加"}</DialogTitle></DialogHeader>
          <div className="grid gap-3">
            {fields.map((f) => (
              <div key={f.key}>
                <label className="text-xs text-muted-foreground mb-1 block">{f.label}{f.required && " *"}</label>
                {f.type === "textarea" ? (
                  <Textarea value={editing?.[f.key] ?? ""} onChange={(e) => set(f.key, e.target.value)} className="bg-card border-border text-foreground" rows={6} />
                ) : f.type === "select" ? (
                  <Select value={editing?.[f.key] ?? ""} onValueChange={(v) => set(f.key, v)}>
                    <SelectTrigger className="bg-card border-border text-foreground"><SelectValue placeholder="選択…" /></SelectTrigger>
                    <SelectContent>{f.options.map((o) => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
                  </Select>
                ) : (
                  <Input type={f.type === "number" ? "number" : f.type === "date" ? "date" : "text"} value={editing?.[f.key] ?? ""} onChange={(e) => set(f.key, f.type === "number" ? Number(e.target.value) : e.target.value)} className="bg-card border-border text-foreground" />
                )}
              </div>
            ))}
          </div>
