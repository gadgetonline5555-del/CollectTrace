    ...
    <div>
      ...
      <div className={`grid gap-5 ${compact ? "md:grid-cols-2 lg:grid-cols-4" : "md:grid-cols-2 xl:grid-cols-4"}`}>
        {PLANS.map((plan) => {
          ...
          return (
            <div key={plan.id} className={`relative rounded-3xl p-6 bg-card/60 backdrop-blur border ${plan.ring} ring-1 flex flex-col ${plan.id === "pro" ? "lg:scale-105 shadow-2xl shadow-violet-500/10" : ""}`}>
              {plan.id === "pro" && (
              ...
              <h3 className="font-display text-2xl font-bold text-foreground">{plan.name[lang]}</h3>
